...
use text_core::{TextProcessingOptions, split_paragraphs, split_sentence_spans_with_abbreviations};

const PHILOSOPHY_ABBREVIATIONS: &[&str] = &["Phys.", "Metaph.", "Eth.", "Rep.", "Bk.", "Ch."];
...
fn sentence_spans(text: &str) -> Vec<SentenceSpan> {
    ...
    let spans = split_sentence_spans_with_abbreviations(text, &options, PHILOSOPHY_ABBREVIATIONS)
        .into_iter()
        .map(|sentence| {
